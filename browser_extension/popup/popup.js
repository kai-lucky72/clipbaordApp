...
copyBtn.addEventListener('click', function() {
    ...
    fetchItemContent(selectedItemId)
        .then(content => {
            ...
            } else if (content.image_data) {
                // For images, we need special handling
                copyImageToClipboard(content.image_data);
            }
        })
        .catch(error => {
...
pasteBtn.addEventListener('click', function() {
    ...
    fetchItemContent(selectedItemId)
        .then(content => {
            if (content.type === 'text') {
                // Send message to background script to paste into active tab
                chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
                    chrome.tabs.sendMessage(tabs[0].id, {
                        ...
                        if (response && response.success) {
                            ...
                            showToast('Failed to paste: ' + (response?.error || 'No valid input field'), 'error');
                        }
                    });
                });
            } else {
                showToast('Can only paste text content', 'error');
            }
        })
        .catch(error => {
...
favoriteBtn.addEventListener('click', function() {
    ...
    fetch(`${API_BASE_URL}/item/${selectedItemId}/favorite`, {
        method: 'POST',
        headers: {
...
deleteBtn.addEventListener('click', function() {
    ...
    if (confirm('Are you sure you want to delete this item?')) {
        // Use the background script to perform the deletion
        chrome.runtime.sendMessage({
            ...
            if (response && response.success) {
                ...
                showToast('Failed to delete item: ' + (response?.error || 'Unknown error'), 'error');
            }
        });
    }
});
...
tagsBtn.addEventListener('click', function() {
    ...
    fetch(`${API_BASE_URL}/item/${selectedItemId}/tags`)
        .then(response => response.json())
        .then(data => {
...
// Manage tags button (same as tagsBtn but checks for item selection)
...
clearHistoryBtn.addEventListener('click', function() {
    if (confirm('Are you sure you want to clear your clipboard history?\nFavorites will be kept unless you uncheck the option below.')) {
        ...
        fetch(`${API_BASE_URL}/items/clear`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
function loadItems() {
    ...
    fetch(`${API_BASE_URL}/items?filter=${currentFilter}&search=${encodeURIComponent(currentSearch)}&page=${currentPage}`)
        .then(response => response.json())
        .then(data => {
        ...
        .catch(error => {
            ...
            itemsList.innerHTML = `
                <div class="empty-preview">
                    ...
                    <p class="small">Please make sure the Clipboard Manager server is running at http://localhost:5000</p>
                </div>
            `;
        });
}
...
function removeTagFromItem(tag) {
    ...
    fetch(`${API_BASE_URL}/item/${selectedItemId}/tags/${encodeURIComponent(tag)}`, {
        method: 'DELETE'
    })
    .then(response => response.json())
    .then(data => {
...
function copyTextToClipboard(text) {
    // Use chrome extension API to copy
    chrome.runtime.sendMessage({
        ...
        canvas.toBlob(function(blob) {
            try {
                ...
                navigator.clipboard.write([
                    ...
                    showToast('Failed to copy image - try copying the image URL instead', 'error');
                });
            } catch (e) {
...
function fetchItemContent(itemId) {
    return fetch(`${API_BASE_URL}/item/${itemId}`)
        .then(response => response.json());
}
